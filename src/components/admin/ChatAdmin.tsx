...
const ChatAdmin: React.FC<ChatAdminProps> = ({
  usuario,
  onClose,
  adminNombre,
}) => {
  ...
  const enviarMensaje = async () => {
    ...
    setEnviando(true);
    await db.enviarMensaje(usuario.id, nuevoMensaje, "admin", adminNombre);
    setNuevoMensaje("");
    setEnviando(false);
  };
  ...
  mensajes.forEach((msg) => {
    ...
    else mensajesAgrupados.push({ fecha: fechaStr, mensajes: [msg] });
  });
        ...
        <div style={styles.modalHeader}>
          <div>
            <h3 style={{ color: "#FF8C42", margin: 0 }}>
              💬 Chat con {usuario.nombre}
            </h3>
            <p
              style={{ fontSize: "11px", color: "#94a3b8", margin: "4px 0 0" }}
            >
              Código: {usuario.codigo} | {usuario.vehiculo || "Sin vehículo"}
            </p>
          </div>
                ...
                {grupo.mensajes.map((msg) => (
                  <div
                    ...
                    style={{
                      ...styles.mensaje,
                      alignSelf:
                        msg.remitente === "admin" ? "flex-end" : "flex-start",
                      backgroundColor:
                        msg.remitente === "admin" ? "#FF8C42" : "#1e293b",
                      color: msg.remitente === "admin" ? "#0f172a" : "#fff",
                    }}
                  >
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  sinMensajes: { textAlign: "center", color: "#94a3b8", padding: "30px 16px" },
  mensaje: {
    maxWidth: "80%",
    padding: "10px 14px",
    borderRadius: "12px",
  ...
  mensajeTexto: { fontSize: "13px", wordBreak: "break-word", lineHeight: 1.4 },
  mensajeImagen: {
    maxWidth: "200px",
    maxHeight: "200px",
    borderRadius: "8px",
  ...
  previewImagen: { width: "100%", borderRadius: "8px" },
  previewTexto: {
    fontSize: "10px",
    color: "#94a3b8",
    textAlign: "center",
